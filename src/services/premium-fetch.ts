...
import { SELF_HOST_WM_KEY } from '@/config/self-host';
...
export async function premiumFetch(
  ...
  // 1b. Self-host build-time enterprise key (web builds). The runtime-config
  //     WORLDMONITOR_API_KEY secret is desktop-only, so on a plain web build we
  //     attach the build-time key here. Must match the server's
  //     WORLDMONITOR_VALID_KEYS. No-op when VITE_WM_KEY is unset (e.g. local dev).
  if (SELF_HOST_WM_KEY) {
    existing.set('X-WorldMonitor-Key', SELF_HOST_WM_KEY);
    const res = await globalThis.fetch(input, { ...withCredentials(init), headers: existing });
    reportServerError(res, input);
    return res;
  }
